...
for run in range(5):  # This loop is for three runs
    ...
    if args.learning=="SSL":
        ...
        for trial in range(1):
            ...
            new_x = None
            new_y = None
            for i in random.sample(np.unique(selected_y).tolist(),len(np.unique(selected_y))):
                count=0
        ...
        count=0
        fl = folder+"/SSL/NonCasCade_" +args.model+ "_" + aug + "_" + args.learning + "_" + str(args.examples) + "_" + str(count) + ".h5"
